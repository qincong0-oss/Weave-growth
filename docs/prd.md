...
版本 v0.2　日期 2026年9月17日　适用对象 产品 设计 研发 试点交付团队
...
## 14 v0.2 增量：外贸回款背调与企业产品接入

本节按产品负责人最新要求扩展 FR03 客户背调与产品资料接入。与 v0.1 概述不一致时，以本节和 ADR 0002 为准。所有企业报告为虚构样例；不授予额度、不执行付款、合同或履约。

### FR17 完整回款背调

新增 `#/diligence`。首页、客户列表和产品适配页提供入口。报告有六个章节：决策总览、主体与采购链、回款与财务、风险与贸易、交易条件、证据与复核。

| 维度 | 展示内容 | 决策边界 |
| --- | --- | --- |
| 身份与采购链 | 法律名称、注册标识、辖区、存续、股权、域名；选料方、下单方、合同买方、付款方 | 品牌/集团/关联实体不自动合并，登记存续不证明付款能力 |
| 本企业回款 | 到期日、核销日、未结清额、逾期额、最长逾期、已结清准时率 | 无交易保持无样本；未付款不进入已结清分母；不同币种不相加 |
| 财务 | 年度、币种、来源及审计；收入趋势、现金流、流动比率、净资产 | 未取得资料不生成数字；买方报表与已核验报告区分 |
| 法律与名单 | 诉讼角色、破产/清算、经营异常、名单及主体匹配 | 未覆盖、无权限、同名待排除与明确结果分开；示例未连接实时数据 |
| 贸易与履约线索 | 采购链、品类、贸易活跃、供应商参考、纺织验收和争议条件 | 海关记录不能证明回款；部分国家与运输方式不覆盖 |
| 付款条件 | 预付、分期、OA、信用证；既有应收加拟议订单未收额；内部额度与保险状态 | 只是未收情景测算，不是预期损失、可用授信或保险赔付承诺 |
| 证据与复核 | 来源、样本日期、范围、有效性、待核实项、责任人、摘要导出 | 外部方法链接不是虚构企业的查询结果 |

Seabrook 演示“新客户资料不足”，North 演示“已有交易且存在逾期”：EUR 13,800 未结清，其中 EUR 4,200 逾期 18 天；两笔已结清，准时率 50%、平均延迟 5 天。所有数字由明确样本计算。30% 预付款的新单 EUR 20,000 对应测算未收 EUR 14,000，与现有应收合计 EUR 27,800；不抵减保险，不计未知其他在制订单。

业务认可、销售交接与账期审批是不同事件。提交财务复核只生成待处理记录，重复提交幂等；交接保留信用待核实状态。企业财务决定真实额度与交易安排。

### FR18 企业产品接入

新增 `#/ingestion`，在首页、产品库和数据连接提供入口。来源分为 ERP/API 示例、实际本地 CSV、规格单提取示例。字段映射可修改；展示逐行校验与旧/新值差异。

料号、名称、成分、克重、幅宽必填。原始记录、来源字段映射、单位归一、重复/缺失错误有明确处理。新产品进入待确认区，冲突保留旧版本并使相关材料失效，异常行不入库。负责人选择采用新值或保留原值，记录来源、原始行号、时间和版本历史；确认不会自动发送旧草稿。

现有产品未提供的可选字段保留旧值；新产品未知参数不补成事实。规格单提取没有真实 OCR；API 没有真实鉴权和定时同步。CSV 实际读取但仅存当前浏览器；上限 100 行 / 256 KB，UTF-8，Excel 需先另存 CSV。详细契约见 `docs/integration-contract.md`。

### 增量验收

| 编号 | 验收 |
| --- | --- |
| AC17 | 缺少财务与交易记录时显示未知，不生成信用良好或回款概率 |
| AC18 | 逾期、部分付款、币种、已结清准时率均按定义计算；商业适配不自动批准信用 |
| AC19 | 不同付款方案改变未收情景，但任何方案都不会自动授信；复核任务幂等 |
| AC20 | CSV 引号、BOM、单位和数据校验有效，文本安全转义，不执行导入内容 |
| AC21 | 字段映射可变更；错误行阻止入库，重复资料不重复导入 |
| AC22 | 冲突仅使相关草稿失效，保留源行与旧版本；拒绝/确认都有可核查结果 |
| AC23 | 纯麻客户不能因料号相同而接受被导入更新成混纺的产品 |
| AC24 | v0.1 浏览器数据可迁移到新 schema，已有沟通记录不丢失；桌面/手机八页面可用 |


## v0.3 订单确认扩展

用户已授权制作基于推荐方案的交互 Demo。完整需求及研究依据见 [订单确认推荐 PRD](order-confirmation-prd.md)，实施范围和边界见 [ADR 0003](decisions/0003-order-confirmation-demo.md)。本文件的售前功能继续保留；先前暂不展开售中售后的阶段限制按 ADR 0003 调整。
