...
export function reduce(s,event){
 ...
 case 'CONFIRM_PRODUCT':{
  const p=n.products.find(x=>x.id===(event.id||'HX-L210'));if(!p)return {state:s,message:'未找到产品'};if(p.confirmed&&!p.conflict)return {state:s,message:'产品资料已确认'};
 ...
 case 'HANDOFF':
  ...
  n.handoffs[event.id]={accepted:true,owner:a.owner,crm:'pending'};n.crmSynced=false;
